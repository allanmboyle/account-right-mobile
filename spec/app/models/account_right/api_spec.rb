describe AccountRight::API do
  ...
  describe ".invoke" do
    ...
    before(:each) { AccountRight::API::RetryingQueryExecutor.stub!(:execute) }
    ...
    it "should return the result of executing the query via the retrying query executor" do
      AccountRight::API::Query.stub!(:new).and_return(query)
      AccountRight::API::RetryingQueryExecutor.should_receive(:execute).with(query).and_return("some json result")
